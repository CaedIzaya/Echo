# 🚨 数据丢失修复 - 优先级清单

## ⚡ 快速总结

**发现问题**：3个关键数据只存储在 localStorage，会丢失！

| 数据 | 问题 | 影响用户感知 |
|------|------|------------|
| 用户经验值 | ❌ 换设备丢失 | 🔴 **高** - 等级重置很明显 |
| 成就列表 | ❌ 换设备丢失 | 🔴 **高** - 成就全部消失 |
| 心树经验 | ❌ 换设备丢失 | 🔴 **高** - 心树等级重置 |

**用户头像**：✅ 已洗清嫌疑（图像数据，特批 localStorage）

---

## 🔥 修复优先级

### Phase 1: 必须立即修复 🔴

**时间**：2-3 小时  
**紧急度**：🔴🔴🔴 非常紧急

#### 1. 用户经验值系统
- **数据库字段**：`userExp`, `userLevel`
- **影响**：用户等级显示在页面顶部，非常显眼
- **用户感知**：⭐⭐⭐⭐⭐ 很明显

#### 2. 成就系统
- **数据库表**：新增 `Achievement` 表
- **影响**：已解锁的成就会消失
- **用户感知**：⭐⭐⭐⭐⭐ 很明显

#### 3. 心树经验状态
- **数据库字段**：`heartTreeExp`, `heartTreeLevel`, `heartTreeCurrentExp`, `heartTreeTotalExp`
- **影响**：心树等级显示在心树页面
- **用户感知**：⭐⭐⭐⭐ 明显

---

### Phase 2: 可以稍后修复 🟡

**时间**：1 小时  
**紧急度**：🟡 中等

#### 4. 统计数据
- **数据库**：可以从 `FocusSession` 重新计算
- **影响**：历史统计图表数据
- **用户感知**：⭐⭐ 不太明显

---

## 📋 修复检查清单

### ✅ 已完成
- [x] 心树名字持久化 - **已修复** 🎉

### ⚠️ 待修复（按优先级）
- [ ] 用户经验值系统 - **Phase 1.1**
- [ ] 成就系统 - **Phase 1.2**
- [ ] 心树经验状态 - **Phase 1.3**
- [ ] 统计数据 - **Phase 2**

---

## 🎯 Phase 1 详细步骤

### Phase 1.1: 用户经验值（30分钟）

#### 1. 修改数据库 Schema
```prisma
model User {
  // 添加字段
  userExp     Float @default(0)
  userLevel   Int   @default(1)
}
```

#### 2. 创建 API
- `src/pages/api/user/exp/update.ts`
- `src/pages/api/user/exp/index.ts`

#### 3. 创建 Hook
- `src/hooks/useUserExp.ts`

#### 4. 更新组件
- `src/pages/dashboard/index.tsx`
- `src/pages/dashboard/index.mobile.tsx`

---

### Phase 1.2: 成就系统（45分钟）

#### 1. 修改数据库 Schema
```prisma
model Achievement {
  id            String   @id @default(cuid())
  userId        String
  achievementId String
  category      String
  unlockedAt    DateTime @default(now())
  user          User     @relation(fields: [userId], references: [id], onDelete: Cascade)
  
  @@unique([userId, achievementId])
  @@index([userId])
}

model User {
  // 添加关系
  achievements Achievement[]
}
```

#### 2. 创建 API
- `src/pages/api/achievements/unlock.ts`
- `src/pages/api/achievements/index.ts`

#### 3. 创建 Hook
- `src/hooks/useAchievements.ts`

#### 4. 更新组件
- `src/lib/AchievementSystem.tsx`
- `src/pages/dashboard/index.tsx`

---

### Phase 1.3: 心树经验状态（45分钟）

#### 1. 修改数据库 Schema
```prisma
model User {
  // 添加字段
  heartTreeLevel      Int      @default(1)
  heartTreeCurrentExp Float    @default(0)
  heartTreeTotalExp   Float    @default(0)
  lastWateredDate     String?
  fertilizerExpiresAt DateTime?
  fertilizerMultiplier Float?
}
```

#### 2. 创建 API
- `src/pages/api/heart-tree/exp/update.ts`
- `src/pages/api/heart-tree/exp/index.ts`

#### 3. 创建 Hook
- `src/hooks/useHeartTreeExp.ts`

#### 4. 更新组件
- `src/lib/HeartTreeExpSystem.ts`
- `src/pages/dashboard/HeartTree.tsx`

---

## 🚀 快速开始命令

```bash
# 1. 查看完整审计报告
cat DATA_PERSISTENCE_AUDIT.md

# 2. 开始修复 Phase 1.1（用户经验值）
# - 修改 prisma/schema.prisma
# - 添加 userExp 和 userLevel 字段

# 3. 应用数据库变更
npm run db:push

# 4. 创建 API 和 Hook
# （按照上面的步骤）

# 5. 测试
# - 获得经验 → 检查数据库
# - 换设备登录 → 验证同步

# 6. 推送代码
git add .
git commit -m "fix: 用户经验值持久化到数据库"
git push origin main
```

---

## 📊 修复进度追踪

| 任务 | 优先级 | 状态 | 预计时间 | 实际时间 |
|------|--------|------|---------|---------|
| 心树名字 | 🔴 | ✅ 完成 | 30min | - |
| 用户经验值 | 🔴 | ⚠️ 待修复 | 30min | - |
| 成就系统 | 🔴 | ⚠️ 待修复 | 45min | - |
| 心树经验 | 🔴 | ⚠️ 待修复 | 45min | - |
| 统计数据 | 🟡 | ⚠️ 待修复 | 60min | - |

**总预计时间**: 约 3-4 小时

---

## 🎁 修复后的好处

### 用户体验
- ✅ 数据永久保存
- ✅ 换设备自动同步
- ✅ 清除浏览器数据不丢失
- ✅ 多设备使用无缝切换

### 技术优势
- ✅ 数据安全可靠
- ✅ 自动迁移旧数据
- ✅ 双重保存策略（速度+安全）
- ✅ 完整的错误处理

---

## 💡 建议

### 立即行动
**建议今天完成 Phase 1**，避免更多用户遇到数据丢失问题。

### 用户通知
修复完成后，建议显示一次性通知：
```
🎉 数据安全升级完成
你的经验、成就、心树等级现在永久保存，
换设备也不会丢失了！
```

### 监控
- 添加数据同步成功率监控
- 记录迁移失败的情况
- 收集用户反馈

---

## 📞 需要帮助？

**查看详细文档**：
- `DATA_PERSISTENCE_AUDIT.md` - 完整审计报告
- `HEART_TREE_NAME_FIX.md` - 心树名字修复案例

**下一步**：
1. 阅读完整审计报告了解详情
2. 按照本文档的优先级开始修复
3. 每完成一项在检查清单中打勾

---

**创建日期**: 2024-12-16  
**状态**: ⚠️ 发现高危问题  
**建议**: 🔴 立即修复 Phase 1







