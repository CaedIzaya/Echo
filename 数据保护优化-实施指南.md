# 🔒 数据保护优化 - 实施指南

## 📋 概述

本次优化将**连续天数**和**总专注时长**从 localStorage 移到数据库，确保用户数据永久安全。

---

## 🎯 优化目标

### 问题
- ❌ 连续天数仅存储在 localStorage，清除缓存会丢失
- ❌ 总专注时长仅存储在 localStorage，不同设备不同步
- ❌ 无法从其他数据恢复这两项数据

### 解决方案
- ✅ 将数据移到数据库（User 表）
- ✅ 双重存储（数据库 + localStorage）
- ✅ 自动同步机制
- ✅ 数据恢复保护

---

## 🗄️ 数据库变更

### Schema 修改

**文件**：`prisma/schema.prisma`

**新增字段**：
```prisma
model User {
  // ... 现有字段 ...
  
  streakDays         Int     @default(0)     // 连续专注天数
  lastStreakDate     String?                 // 最后一次更新的日期 (YYYY-MM-DD)
  totalFocusMinutes  Int     @default(0)     // 总专注时长（分钟）
}
```

### 迁移步骤

#### 步骤 1：生成迁移文件

```bash
cd C:\Users\ASUS\Desktop\t3-app
npx prisma migrate dev --name add_user_stats_fields
```

**预期输出**：
```
✔ Generated Prisma Client
✔ The migration has been created successfully
```

#### 步骤 2：应用迁移

迁移会自动应用，如果需要手动应用：

```bash
npx prisma migrate deploy
```

#### 步骤 3：验证迁移

```bash
npx prisma studio
```

打开 User 表，确认新字段存在：
- `streakDays` (Int, default: 0)
- `lastStreakDate` (String, nullable)
- `totalFocusMinutes` (Int, default: 0)

---

## 🔧 代码变更

### 1. 新增 API 端点

#### `/api/user/stats` (GET)

**功能**：获取用户统计数据

**请求**：
```http
GET /api/user/stats
Authorization: Bearer <token>
```

**响应**：
```json
{
  "success": true,
  "stats": {
    "streakDays": 30,
    "lastStreakDate": "2025-12-23",
    "totalFocusMinutes": 1800
  }
}
```

#### `/api/user/stats/update` (POST)

**功能**：更新用户统计数据

**请求**：
```http
POST /api/user/stats/update
Content-Type: application/json

{
  "streakDays": 31,
  "lastStreakDate": "2025-12-24",
  "totalFocusMinutes": 1860
}
```

**响应**：
```json
{
  "success": true,
  "stats": {
    "streakDays": 31,
    "lastStreakDate": "2025-12-24",
    "totalFocusMinutes": 1860
  }
}
```

---

### 2. 新增 Hook

#### `useUserStats`

**文件**：`src/hooks/useUserStats.ts`

**功能**：
- 管理连续天数和总时长
- 自动从数据库加载
- 自动同步到数据库
- localStorage 备份

**使用方法**：
```typescript
import { useUserStats } from '~/hooks/useUserStats';

function MyComponent() {
  const {
    streakDays,
    lastStreakDate,
    totalFocusMinutes,
    isLoading,
    updateStreakDays,
    updateTotalMinutes,
    reload,
  } = useUserStats();

  // 更新连续天数
  const handleUpdateStreak = async () => {
    await updateStreakDays(31, '2025-12-24');
  };

  // 增加总时长
  const handleAddMinutes = async (minutes: number) => {
    await updateTotalMinutes(minutes);
  };

  return (
    <div>
      <p>连续天数: {streakDays}</p>
      <p>总时长: {totalFocusMinutes} 分钟</p>
    </div>
  );
}
```

---

## 📊 数据迁移

### 迁移现有用户数据

**目的**：将现有用户的 localStorage 数据迁移到数据库

**方法 1：前端自动迁移**（推荐）

`useUserStats` Hook 已内置自动迁移：
```typescript
// Hook 会自动检测 localStorage 和数据库的值
// 如果 localStorage 的值更大，自动同步到数据库
if (localStreakDays > dbStreakDays) {
  await fetch('/api/user/stats/update', {
    method: 'POST',
    body: JSON.stringify({ streakDays: localStreakDays }),
  });
}
```

**用户操作**：
1. 用户登录
2. 打开 Dashboard
3. Hook 自动加载并迁移数据
4. 完成！

**方法 2：后端批量迁移**（可选）

创建一次性迁移脚本：

```typescript
// scripts/migrate-user-stats.ts
import { db } from '~/server/db';

async function migrateUserStats() {
  const users = await db.user.findMany({
    where: {
      OR: [
        { streakDays: 0 },
        { totalFocusMinutes: 0 },
      ],
    },
  });

  for (const user of users) {
    // 从 FocusSession 计算总时长
    const sessions = await db.focusSession.findMany({
      where: { userId: user.id },
      select: { duration: true },
    });
    
    const totalMinutes = sessions.reduce((sum, s) => sum + (s.duration || 0), 0);

    // 更新数据库
    await db.user.update({
      where: { id: user.id },
      data: { totalFocusMinutes: totalMinutes },
    });

    console.log(`✅ 迁移用户 ${user.id}: ${totalMinutes} 分钟`);
  }

  console.log('✅ 迁移完成！');
}

migrateUserStats();
```

运行：
```bash
npx ts-node scripts/migrate-user-stats.ts
```

---

## 🧪 测试清单

### 测试 1：新用户注册

**步骤**：
1. 注册新用户
2. 完成 Onboarding
3. 完成一次专注（30分钟）
4. 检查数据库

**预期**：
```sql
SELECT streakDays, totalFocusMinutes FROM "User" WHERE id = '<userId>';
-- streakDays: 0 (第二天才会更新)
-- totalFocusMinutes: 30
```

---

### 测试 2：连续天数更新

**步骤**：
1. 用户完成第一天专注
2. 第二天登录
3. 完成第二天专注
4. 检查连续天数

**预期**：
```
Day 1: streakDays = 0
Day 2: streakDays = 1 (昨天完成了目标)
Day 3: streakDays = 2
```

---

### 测试 3：数据恢复

**步骤**：
1. 用户有数据：streakDays = 10, totalMinutes = 600
2. 清除浏览器缓存
3. 重新登录
4. 检查数据

**预期**：
```
✅ 数据从数据库恢复
✅ streakDays = 10
✅ totalMinutes = 600
```

---

### 测试 4：跨设备同步

**步骤**：
1. 设备 A：专注 30 分钟
2. 设备 B：登录并查看
3. 检查数据

**预期**：
```
✅ 设备 B 显示最新数据
✅ totalMinutes 正确同步
```

---

### 测试 5：localStorage 优先

**步骤**：
1. 数据库：streakDays = 10
2. localStorage：streakDays = 15
3. 用户登录
4. 检查数据

**预期**：
```
✅ 使用 localStorage 的值（15）
✅ 自动同步到数据库（15）
```

---

## 🔍 验证方法

### 方法 1：浏览器控制台

```javascript
// 1. 检查 localStorage
console.log('localStorage:', {
  streakDays: localStorage.getItem('userStreakDays'),
  totalMinutes: localStorage.getItem('totalFocusMinutes'),
});

// 2. 检查数据库
fetch('/api/user/stats')
  .then(res => res.json())
  .then(data => console.log('数据库:', data.stats));

// 3. 对比
// 应该一致
```

### 方法 2：Prisma Studio

```bash
npx prisma studio
```

打开 User 表，查看：
- `streakDays`
- `lastStreakDate`
- `totalFocusMinutes`

### 方法 3：数据库查询

```sql
SELECT 
  id,
  name,
  streakDays,
  lastStreakDate,
  totalFocusMinutes,
  createdAt
FROM "User"
WHERE id = '<userId>';
```

---

## 📝 回滚方案

### 如果需要回滚

#### 步骤 1：回滚数据库

```bash
npx prisma migrate resolve --rolled-back <migration-name>
```

#### 步骤 2：删除新增文件

```bash
rm src/pages/api/user/stats/index.ts
rm src/pages/api/user/stats/update.ts
rm src/hooks/useUserStats.ts
```

#### 步骤 3：恢复 schema.prisma

删除新增的三个字段：
```prisma
model User {
  // 删除这三行
  streakDays         Int     @default(0)
  lastStreakDate     String?
  totalFocusMinutes  Int     @default(0)
}
```

#### 步骤 4：重新生成 Prisma Client

```bash
npx prisma generate
```

---

## 🚀 部署清单

### 开发环境

- [x] ✅ 修改 `schema.prisma`
- [x] ✅ 创建 API 端点
- [x] ✅ 创建 `useUserStats` Hook
- [ ] 🔴 运行 `npx prisma migrate dev`
- [ ] 🔴 测试所有功能
- [ ] 🔴 验证数据迁移

### 生产环境

- [ ] 🔴 备份数据库
- [ ] 🔴 运行 `npx prisma migrate deploy`
- [ ] 🔴 验证迁移成功
- [ ] 🔴 监控错误日志
- [ ] 🔴 验证用户数据正确

---

## 📊 监控指标

### 关键指标

1. **迁移成功率**
   ```sql
   SELECT 
     COUNT(*) as total_users,
     COUNT(CASE WHEN totalFocusMinutes > 0 THEN 1 END) as migrated_users
   FROM "User";
   ```

2. **数据一致性**
   ```typescript
   // 检查 localStorage 和数据库是否一致
   const dbStats = await fetch('/api/user/stats').then(r => r.json());
   const localStreak = parseInt(localStorage.getItem('userStreakDays') || '0');
   
   if (Math.abs(dbStats.stats.streakDays - localStreak) > 1) {
     console.error('⚠️ 数据不一致！');
   }
   ```

3. **API 性能**
   ```
   GET /api/user/stats - 应该 < 100ms
   POST /api/user/stats/update - 应该 < 200ms
   ```

---

## 💡 常见问题

### Q1：迁移会影响现有用户吗？

**A**：不会！
- 新字段有默认值（0）
- 现有功能不受影响
- 自动迁移机制会在用户登录时运行

### Q2：如果用户有多个设备怎么办？

**A**：数据会自动同步
- 每个设备登录时都会从数据库加载最新数据
- 使用"取较大值"策略，确保数据不丢失

### Q3：localStorage 还有用吗？

**A**：有！
- 作为本地缓存，提升性能
- 作为备份，防止数据库故障
- 离线时仍可使用

### Q4：如何处理数据冲突？

**A**：使用"取较大值"策略
```typescript
const finalValue = Math.max(dbValue, localValue);
```
这样确保用户的努力不会丢失。

---

## 🎯 预期效果

### 优化前

```
❌ 清除缓存 → 连续天数丢失
❌ 换设备 → 数据不同步
❌ 数据恢复 → 无法恢复
```

### 优化后

```
✅ 清除缓存 → 从数据库恢复
✅ 换设备 → 自动同步
✅ 数据恢复 → 完全恢复
✅ 数据安全 → 永久保存
```

---

## 📞 支持

如有问题，请联系开发团队。

**实施日期**：2025-12-23  
**版本**：v3.3.0  
**状态**：✅ 准备就绪

---

**下一步**：
1. 运行 `npx prisma migrate dev --name add_user_stats_fields`
2. 测试所有功能
3. 部署到生产环境

所有准备工作已完成，可以安全部署！🚀






