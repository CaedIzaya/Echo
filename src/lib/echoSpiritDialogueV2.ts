...
// 🌱 动态状态池：根据用户当前状态动态融合进日常交互
// 这些文案会根据心树等级和心流指数，自动merge到通用人格池中

// 心树等级专属文案
export const heartTreeLevelPools = {
  // 🌱 0-10级：新手期，鼓励持续、不急于求成
  level_0_10: [
    '曾经的心树，也只是一颗小苗呢。',
    '你现在做的这些，看起来不大，但很重要。',
    '刚开始的时候，能持续出现，本身就是一种能力。',
    '不是所有变化都要立刻被看见，有些在地下生长。',
    '你在慢慢建立属于自己的节奏，这比快更难。',
    '别小看现在的样子，它已经和昨天不一样了。',
    '有些人还没开始，而你已经在路上了。',
    '心树现在还不高，但根已经在伸展了。',
  ...
  // 🌿 10-20级：成长期，认可进步、稳定节奏
  level_10_20: [
    '你已经不是在找感觉了，你开始拥有感觉。',
    '这段时间的专注，有点稳。不是偶然那种。',
    '我注意到你进入状态的速度变快了。',
    '现在的你，看起来像是在走自己的节奏，而不是追着谁跑。',
    '这不是一两次的灵光，而是一段持续的状态。',
    '你开始知道什么时候该用力，什么时候该放松。',
    '有些成果已经在累积了，只是它们不吵不闹。',
    '这条路还很长，不过你已经学会怎么走了。',
    '你现在的专注，看起来很踏实。',
    '不需要加速，现在这个步伐刚刚好。',
...
// 心流指数专属文案
export const flowIndexPools = {
  // 🌫️ 心流指数 < 50：起步期，降低门槛、温和鼓励
  flow_low: [
    '没关系，所有开始看起来都像这样。',
    '现在不用想太多，先动起来就好。',
    '不是状态不好，只是还没完全进入。',
...
// 等级成长事件文案（作为独立事件触发，优先级 MEDIUM）
export const levelUpEventPool = [
  '哇哦！灵光一闪，看来你又成长了呢。',
  '升级了。不是变强那种，是变得更懂自己那种。',
  '你刚刚悄悄往前走了一步。我看见了。',
  '等级+1。别急着庆祝，先记住这份稳定的手感。',
  '又解锁了一点新视角。你在把自己打磨得更清晰。',
];
...
/**
 * 根据用户当前状态，动态构建通用人格池
 * @param heartTreeLevel 心树等级
 * @param flowIndex 心流指数 (0-100)
 * @returns 融合了状态专属文案的通用池
 */
export function buildDynamicUniversalPool(
  heartTreeLevel: number = 0,
  flowIndex: number = 0,
): string[] {
  const basePool = [...defaultEchoPools.universal_pool];
  
  // 根据心树等级添加对应文案
  let levelPool: string[] = [];
  if (heartTreeLevel < 10) {
    levelPool = heartTreeLevelPools.level_0_10;
  } else if (heartTreeLevel < 20) {
    levelPool = heartTreeLevelPools.level_10_20;
  } else {
    levelPool = heartTreeLevelPools.level_20_plus;
  }
  
  // 根据心流指数添加对应文案
  let flowPool: string[] = [];
  if (flowIndex < 50) {
    flowPool = flowIndexPools.flow_low;
  } else if (flowIndex < 70) {
    flowPool = flowIndexPools.flow_medium;
  } else {
    flowPool = flowIndexPools.flow_high;
  }
  
  // 融合：基础池 + 等级池 + 心流池
  return [...basePool, ...levelPool, ...flowPool];
}

/**
 * 小精灵被点按时，根据用户状态动态选择文案
 * @param options.heartTreeLevel 心树等级
 * @param options.flowIndex 心流指数
 * @param options.pools 可选的自定义池
 */
export function pickUniversalSentence(
  ...
  const heartTreeLevel = options?.heartTreeLevel ?? 0;
  const flowIndex = options?.flowIndex ?? 0;
  const customPools = options?.pools;
  
  // 构建动态池
  const dynamicPool = buildDynamicUniversalPool(heartTreeLevel, flowIndex);
  
  // 如果有自定义池，合并
  const mergedPools: EchoPools = { 
    ...defaultEchoPools, 
    ...(customPools ?? {}),
    universal_pool: dynamicPool,
  };
 ...
 * 等级提升事件文案
 */
export function pickLevelUpSentence(): EchoSentenceResult {
  const idx = Math.floor(Math.random() * levelUpEventPool.length);
  return {
    text: levelUpEventPool[idx],
    pool: 'level_up_event',
  };
}
...
export function pickUniversalSentenceWithAwareness(
  ...
  const fallback = pickUniversalSentence({ pools: options?.pools });
  return { ...fallback, source: 'ECHO' };
}
